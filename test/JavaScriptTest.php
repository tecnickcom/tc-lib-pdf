...
class JavaScriptTest extends TestUtil
{
    ...
    public function testAppendRawJavaScriptAppendsScript(): void
    {
        ...
        $this->assertSame('var a = 1;var b = 2;', $this->getObjectProperty($obj, 'javascript'));
    }

    public function testAddRawJavaScriptObjAddsObjectAndReturnsId(): void
    {
        $obj = $this->getTestObject();
        $id = $obj->addRawJavaScriptObj('app.alert("ok");', true);

        $this->assertGreaterThan(0, $id);
        $list = $this->getObjectProperty($obj, 'jsobjects');
        $this->assertCount(1, $list);
        $this->assertSame($id, $list[0]['n']);
    ...
    public function testAddRawJavaScriptObjReturnsMinusOneInPdfaMode(): void
    {
        ...
        $id = $obj->addRawJavaScriptObj('app.alert("no");');
        $this->assertSame(-1, $id);
        $this->assertCount(0, $this->getObjectProperty($obj, 'jsobjects'));
    }
    ...
    public function testXObjectTemplateLifecycleAndMutators(): void
    {
        ...
        $this->assertSame('', $this->getObjectProperty($obj, 'xobjtid'));
    ...
    {
        $obj = $this->getTestObject();
        $a1 = $obj->setAnnotation(1, 2, 3, 4, 'note', ['subtype' => 'Text']);
        $a2 = $obj->setLink(5, 6, 7, 8, '#dest');

        $ann = $this->getObjectProperty($obj, 'annotation');
        $this->assertArrayHasKey($a1, $ann);
        $this->assertArrayHasKey($a2, $ann);
        $this->assertSame('note', $ann[$a1]['txt']);
        $this->assertSame('Link', $ann[$a2]['opt']['subtype']);
    }
    ...
    public function testAddJSFieldWrappersAppendExpectedScripts(): void
    {
        ...
        $handler = \set_error_handler(static function (int $errno, string $errstr): bool {
        ...
        $js = $this->getObjectProperty($obj, 'javascript');
        $this->assertStringContainsString("fbtn.buttonSetCaption('Go');", $js);
        $this->assertStringContainsString("fbtn.setAction('MouseUp'", $js);
        $this->assertStringContainsString("fchk=this.addField('chk','checkbox'", $js);
        $this->assertStringContainsString("fcmb.setItems(", $js);
        $this->assertStringContainsString("flst.\\setItems(", $js);
        $this->assertStringContainsString("frad=this.addField('rad','radiobutton'", $js);
        $this->assertStringContainsString("ftxt=this.addField('txt','text'", $js);
    }
}
